# @stealthscale/component-layout

Arranges what is already there: the boxes, the stacks, the rules between them and the surfaces under
them. Every component binds a recipe and draws nothing of its own, so a theme moves all of them by
extending the recipe. The preset under `./theme` registers the recipes with an application's
compiler.

## Install

```bash
pnpm add @stealthscale/component-layout
```

The package peers on `react` and `@stealthscale/theme`.
