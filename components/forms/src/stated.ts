/**
 * Drops the settings a caller left unset, so a machine or a recipe reads only what was stated.
 *
 * @remarks
 *   A machine takes the settings it defaults without `undefined`, and its own splitter hands every
 *   setting over carrying it. A generated variant is optional in the same way. Passing either
 *   straight through offers a value the type refuses, and the list of which settings default is the
 *   machine's own and different for each one. This drops every setting nobody set, which leaves the
 *   machine and the recipe to apply their own defaults.
 */
 ...
 *
 * @typeParam Options - The settings a caller handed over.
 * @param options - Everything a splitter or a component took out of the props.
