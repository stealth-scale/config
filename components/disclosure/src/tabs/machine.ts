 ...
 *
 * @remarks
 *   The three settings the machine defaults are spread only where the caller named one. It takes
 *   them without `undefined`, and its own splitter hands them over with it, so passing them
 *   straight through offers the machine a value it refuses. The collapsible needs none of this: it
 *   defaults nothing.
...
export function useTabsMachine(options: TabsOptions): TabsApi {
  ...
  const { activationMode, loopFocus, orientation, ...rest } = options;

  return tabs.connect(
    useMachine(tabs.machine, {
      ...rest,
      ...(activationMode === undefined ? {} : { activationMode }),
      ...(loopFocus === undefined ? {} : { loopFocus }),
