...
| `source.specs`     | Every kind | A file under `src` with no `.spec.ts` or `.spec.tsx` beside it. A fixture, a declaration file and a module whose every export is an `export type` are left alone, none of them holding behaviour of its own, and so is a barrel unless the package asks for barrels with `barrels: true`                                   |
| `source.declared`  | Every kind | A package a file under `src` imports that the manifest lists under neither `dependencies` nor `peerDependencies`. A subpath reads as the package that publishes it, and a relative path, a `#` subpath and a `node:` builtin are read past. A specification and a fixture are read past too, both running in the workspace |
