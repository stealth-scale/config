...
own conformance spec all reach for the same three.

A spec that reads the real workspace states the rule and derives its expectation from the tree. A
spec that writes its own scratch workspace in `tmpdir` may pin exact counts and names, because it
wrote them.
