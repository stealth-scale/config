 ...
 * else, and a root takes the node tier whatever its packages render. The tier that knows about
 * `.tsx` specifications is therefore never the one the linter reads, and without this they are
 * held to a standard the same file in `.ts` is excused.
 *
