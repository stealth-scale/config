 ...
 * answer for `import "./panel.css"` — it is not a module — so the import throws the moment the
 * server reaches it.
 *
 ...
 */
export function bundled(stated: Bundled): readonly Contribution[] {
  return stated.deps.map((held) =>
    contribute({ at: AT, because: stated.because, item: held, name: `ssr.bundled(${held})` }),
  );
}
