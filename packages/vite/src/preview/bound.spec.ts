...
test("listens on the address it was given", () => {
  expect((bound("127.0.0.1").config as UserConfig).preview?.host).toBe("127.0.0.1");
});

test("listens on every interface where that is what was asked for", () => {
  expect((bound(true).config as UserConfig).preview?.host).toBe(true);
});

test("leaves the dev server alone, which binds its own address", () => {
  expect((bound("127.0.0.1").config as UserConfig).server).toBeUndefined();
});
