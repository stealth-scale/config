...
/**
 * Draws a component that meets the whole contract: it marks its part, lets a caller's class win,
 * takes a reference and keeps an inline style.
 *
 * A design system lets the class win by merging through `cn`, so a caller's `rounded-none`
 * displaces the component's `rounded-lg` rather than joining it. Replacing outright is the same
 * thing for one colliding pair, and it keeps a Tailwind library out of the toolchain for the sake
 * of a fixture.
 *
 * @param props - The class the suite sets, and every DOM prop it spreads onto the part.
 * @returns The marked part.
 */
...
/**
 * Draws a component that sends the class to a wrapper and everything else to the control, which is
 * the split `stylesSlot` exists for.
 *
 * It also answers neither the reference nor the style, standing in for a component whose root is a
 * function component that never declared a reference, or whose inline styles are written by the
 ...
 *
 * @param props - The class the suite sends to the wrapper, and the title it sends to the control.
 * @returns The wrapper, around the control.
 */
