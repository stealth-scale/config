 ...
 *
 * @typeParam E - The element the component's part renders as.
 */
...
export interface ContractSpec<E extends Element> {
  ...
  /**
   * `[consumer, internal]` — two Tailwind utilities that collide. The consumer's must win, which
   * only happens when the component merges through `cn` rather than concatenating.
   */
  overrides: [consumer: string, internal: string];

  /**
   * Skip the `ref` assertion, for a component whose root is a third-party function component that
   * never declared a `ref` prop. `Calendar` renders react-day-picker's `DayPicker`, which is not a
   * forwardRef component, so a ref has nowhere to land. Set this only with that kind of reason — a
   * missing ref in _our_ wrapper is a defect to fix, not to skip.
   */
  skipRef?: boolean;

  /**
   * Skip the `style` assertion, for a control whose inline styles are computed by the library that
   * owns it. `input-otp` positions its single real `<input>` as a transparent overlay with inline
   * styles applied after the consumer's, so a passed `style` cannot survive — a fact about that
   * library, not a defect in the wrapper. Set this only with that kind of reason.
   */
  skipStyle?: boolean;

  /**
   * Names the `data-slot` on the part that receives the reference, the style and any arbitrary
   * attribute.
   */
  slot: string;
   ...
   * `<select>` so it can overlay a chevron — and send `className` to the wrapper while everything
   * else spreads onto the control. That split is deliberate and worth asserting rather than
   * glossing over, so the two slots can differ.
   */
  stylesSlot?: string;
}
 ...
 * reference that stops at a wrapper, a missing slot. They are the same shape for every component,
 * so they are written here once rather than copied into each spec.
 *
 * The assertions are plain reads of the document rather than jest-dom, so this file carries no
 * matcher setup of its own and still fails with a readable difference.
 *
 * @typeParam E - The element the component's part renders as.
 * @param spec - How to render it and where its part is. `ContractSpec` documents every member.
 */
export function describeContract<E extends Element>(spec: ContractSpec<E>): void {
   ...
   * @returns The element carrying the slot the spec named.
   */
   ...
   *
   * @param container - The rendered output.
   * @returns The element the class was sent to.
   */
