...
describe("ROLES", () => {
  it("places a table for every hue Steel draws", () => {
  ...
  it("places the solid of blue where the gate accepts it", () => {
    expect(ROLES.blue.solid).toStrictEqual([60, 60]);
  });

  it("writes the text on a solid as a color the foundation names", () => {
    expect(ROLES.blue.contrast).toStrictEqual({
      value: { _dark: "{colors.gray.white}", base: "{colors.gray.white}" },
    });
  });

  it("places the page on the neutral ramp in both modes", () => {
    expect(SURFACES.DEFAULT).toStrictEqual(["white", 100]);
  });

  it("places the default ink and line", () => {
    expect(INKS.DEFAULT).toStrictEqual([100, 10]);
    expect(LINES.DEFAULT).toStrictEqual([30, 70]);
  });
});
