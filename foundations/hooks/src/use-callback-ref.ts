 ...
 *
 * @remarks
 *   An effect that lists a handler in its dependencies re-runs whenever the caller passes a new
 *   closure, which is every render for anything written inline. Listing this function instead
 *   re-runs the effect only when its other dependencies change. The ref is written in an insertion
 *   effect rather than during the render, because a render may be thrown away and a discarded one
 *   must not leave the ref holding a callback that never mounted.
 ...
 * @param deps - The values the stable function's identity changes with. Passing none keeps one
