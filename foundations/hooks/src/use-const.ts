 ...
 *
 * @remarks
 *   State without a setter is the only thing React promises to build once. `useMemo` is a cache the
 *   runtime may drop and rebuild, so a caller that compares by identity cannot use it. Use this for
 *   a collator, an observer, or an object used as a key.
 ...
 *   whose value is itself a function returns it from a factory.
