...
- `Link` draws words a person follows to somewhere else. The ink, the visited ink, the cursor and
  ...
  under a pointer, both looks underlining under one.
...
- The landmark is named `Breadcrumb` by default, a page usually holding more than one navigation
  landmark and an unnamed one being announced with nothing to tell it from the others. The list
  states its own list role, because a list drawn with no marker loses that role in Safari. The
