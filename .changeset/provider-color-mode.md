...
provider-color-mode: add the provider that puts a colour mode in scope
...
- `useColorMode` answers the way the page is drawn, the choice behind it, and the setter. It throws
  where there is no provider above it, because a hook answering a default would let a subtree draw
...
- `useSystemColorMode` reads the machine's own setting through `useSyncExternalStore` and follows it
  as it changes. It answers `light` on a server, which no first paint rests on.
