...
- `ColorModeProvider` and `ThemeProvider` both own `data-color-mode` on the document root, and the
  ...
  and passes it on, so the two write the same thing rather than undoing each other. A person
  following the machine is passed nothing, which is how both spell that.
- `store` reaches every setting, so a specification hands one memory store and the page's local
