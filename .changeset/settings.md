---
"@stealthscale/settings": minor
---

settings: add the setting a person chose, kept where the reader chooses
...
- `defineSetting` declares one setting: its name, the values it may take, the value it falls back
  ...
  that follows the machine until somebody overrides it says both without repeating either.
...
- `SettingStore` asks for `subscribe` beside read and write. The browser's own `Storage` interface
  reports nothing, which is why a setting built on it alone cannot tell one tab what another chose.
...
- `cookieStore()` keeps settings in a cookie, which a server reads from the request header before it
  ...
  `document.cookie`, which answers at once, and the Cookie Store API reports a change where the
  browser ships it.
